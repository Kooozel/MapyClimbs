/**
 * map-inject.js — Content Script (v0.5.5)
 * SPA lifecycle controller: GPX polling, map overlay, button/panel injection.
 * Depends on (loaded before this file by manifest.json):
 *   gpx-parser.js          -> parseGPX()
 *   map-inject-chart.js    -> generateElevationChart()
 *   map-inject-panel.js    -> buildPanel(), showChartOverlay(), hideChartOverlay()
 */
/* global parseGPX, buildPanel */
...
(function () {
  ...
  function init() {
    ...
    chrome.storage.local.get(["lastClimbResult"], (data) => {
      if (
        data.lastClimbResult &&
        Array.isArray(data.lastClimbResult) &&
        data.lastClimbResult.length &&
        !data.lastClimbResult[0].markerCoords
      ) {
    ...
    // Poll URL at 150 ms - Mapy.cz calls replaceState on pan/zoom, patching
    // replaceState is unreliable; polling is simpler.
    let _lastURL = "";
  ...
  function pollForGPX() {
    chrome.storage.local.get(["pendingGPX", "lastClimbResult", "lastTotalDistance"], (data) => {
      ...
      // Restore from cache (e.g. page reload)
  ...
  function renderMapOverlay() {
    ...
    _climbs.forEach((climb, i) => {
      ...
      // Start pin: circle with climb number
      if (climb.markerCoords) {
        ...
        if (s.x >= -25 && s.x <= mb.width + 25 && s.y >= -25 && s.y <= mb.height + 25) {
          ...
          pin.innerHTML =
            ...
            ' paint-order="stroke" stroke="#000" stroke-width="1.5" opacity="0.8">' +
            (i + 1) +
            "</text>" +
            "</svg>";
      ...
      // End pin: mountain silhouette with category label
      if (climb.endCoords) {
        ...
        if (e.x >= -35 && e.x <= mb.width + 35 && e.y >= -40 && e.y <= mb.height + 10) {
          ...
          peak.innerHTML =
            ...
            i +
            ')">' +
            '<path d="M460 320 H177 c-3 0-5-3-4-6 l90-184 112 2 89 182 c1 3-1 6-4 6z" fill="' +
            color +
            ...
            '<text x="500" y="280" font-family="Arial, sans-serif" font-weight="900" font-size="160" fill="' +
            color +
            '" stroke="#000" stroke-width="8" style="paint-order: stroke fill;"><tspan>' +
            catLabel +
            "</tspan></text>" +
            "</g>" +
            "</svg>";
   ...
   * Falls back to full viewport when no canvas is found.
   */
  ...
  function onClimbButtonClick() {
    ...
    // Observe for the save/export modal and suppress it + its download
   ...
   * Primary: .icon-action[title="Export"] > button (confirmed DOM shape).
