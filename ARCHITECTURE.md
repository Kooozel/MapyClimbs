...
| File | What it is |
| --- | --- |
| `climb-engine.ts` | The detection pipeline. Pure — no Chrome APIs, no DOM. Five steps. |
| `climb-engine.config.ts` | Every numeric threshold the pipeline uses. Tune here, not in the logic. |
| `scoring.ts` | Scoring models (`aso`, `garmin`, hiking) and their category threshold tables. |
...
| `geo.ts` | The one haversine. Both GPX readers accumulate distance with it. |
| `gpx-parser.ts` | Browser GPX reader (`DOMParser`) → `ElevationTuple[]`. |
...
| `types.ts` | Domain types, `StorageKey`, and the `chrome.runtime` message/response union. |
...
| `garmin-gpx.ts` | Node-side GPX reader. Exists alongside `src/gpx-parser.ts` because ride analysis needs `<time>` and heart rate, which the browser reader discards. |
...
| `garmin-gpx.test.js`, `ride-metrics.test.js`, `ride-analysis.test.js` | The CLI layer. |
...
| `scripts/build-cli.mjs` | esbuild bundle of the engine + CLI into `dist-cli/`, kept out of `dist/`. |
