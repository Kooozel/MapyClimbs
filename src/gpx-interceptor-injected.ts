/**
 * gpx-interceptor-injected.ts — Page-context script (web_accessible_resource).
 * Intercepts fetch/XHR GPX export requests and captures the SMap instance.
 * Communicates with the content script (interceptor.ts) via postMessage.
 *
 * This file runs in the page's JavaScript context (not a content script
 * sandbox), so it can patch window.fetch and XMLHttpRequest prototypes.
 */
